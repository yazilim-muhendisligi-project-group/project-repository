...
public class ProductDAO {
    ...
    public boolean updateProductDetails(int id, String name, String category, double price, String unit) {
        final String SQL = "UPDATE products SET name = ?, category = ?, price = ?, unit = ? WHERE id = ?";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            int affectedRows = stmt.executeUpdate();
            if (affectedRows > 0) {
                LOGGER.info("Urun bilgileri guncellendi: ID=" + id);
                return true;
            }

        } catch (SQLException e) {
            LOGGER.log(Level.SEVERE, "Urun guncellenirken hata: ID=" + id, e);
        }
        return false;
    }
}
